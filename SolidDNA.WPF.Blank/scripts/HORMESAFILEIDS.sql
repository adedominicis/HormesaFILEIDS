...
(
	...
	CONSTRAINT [PK_LogEvents] PRIMARY KEY CLUSTERED ([Id] ASC) WITH (
		PAD_INDEX = OFF,
...
CREATE VIEW listadoArchivosActivos
AS
	SELECT format(archivos.id, '000-000') AS 'PARTID',
		fullpath AS 'RUTA'
	FROM archivos
go
...
go
use HORMESAFILEIDS
grant select to FILEIDS
grant insert to FILEIDS
grant update to FILEIDS
grant delete to FILEIDS
grant execute to fileids